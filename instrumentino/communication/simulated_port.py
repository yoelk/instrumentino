...
#     sim_data_pattern = [30,70]*10
    ...
    def _get_incoming_bytes(self):
        ...
        if self.reply_to_ping:
            packet_length = (ControlinoProtocol.packet_header_format.sizeof() + 
                             len(self.controller.controlino_protocol.STRING_PACKET_TYPE_PONG))
        
            sim_string_packet = self.controller.controlino_protocol.string_packet_format.build(Container(const_header=ControlinoProtocol.CONST_HEADER,
                                                                                                         type='STRING_PACKET',
            ...
            return sim_string_packet
        
        # Start counting the packet's length, to be put later in the packet.
        packet_length = (ControlinoProtocol.packet_header_format.sizeof() + 
                         ControlinoProtocol.data_packet_header_format.sizeof() +
                         ControlinoProtocol.data_packet_blocks_num_format.sizeof())
        ...
        for idx, channel in enumerate(self.controller.input_channels):
            ...
            packet_length += len(block_serialized)
        ...
        sim_data_packet = self.controller.controlino_protocol.data_packet_format.build(Container(const_header=ControlinoProtocol.CONST_HEADER,
                                                                                                 type='DATA_PACKET',
                                                                                                 packet_length=packet_length,
