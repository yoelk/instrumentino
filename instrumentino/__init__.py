...
class InstrumentinoApp(App):
    ...
    def __init__(self, **kwargs):
        ...
        self.action_classes = [c for c in sys.modules[self.instrument_module].__dict__.values() if
                               isclass(c) and issubclass(c, Action) and c is not Action]
    ...
    def build_config(self, config):
        ...
        for controller in self.controllers:
            config.set(self.SETTINGS_COMM_MENU, self.create_settings_key(controller.name, self.SETTINGS_KEY_COMM_TYPE),
                       list(CommunicationTypesLoader.get_comm_types().keys())[0])
            config.set(self.SETTINGS_COMM_MENU,
                       self.create_settings_key(controller.name, self.SETTINGS_KEY_COMM_ADDRESS), '')
            config.set(self.SETTINGS_COMM_MENU,
                       self.create_settings_key(controller.name, self.SETTINGS_KEY_COMM_STATUS), '0')
    ...
    def build_settings(self, settings):
        ...
        for controller in self.controllers:
            ...
            comm_type = self.config.get(self.SETTINGS_COMM_MENU,
                                        self.create_settings_key(controller.name, self.SETTINGS_KEY_COMM_TYPE))
        ...
        if section == self.SETTINGS_COMM_MENU:
            ...
            if setting_name == self.SETTINGS_KEY_COMM_STATUS:
                if value == '0':
                    ...
                    address = self.config.get(self.SETTINGS_COMM_MENU,
                                              self.create_settings_key(controller.name, self.SETTINGS_KEY_COMM_ADDRESS))
                    comm_type = self.config.get(self.SETTINGS_COMM_MENU,
                                                self.create_settings_key(controller.name, self.SETTINGS_KEY_COMM_TYPE))
                    comm_class = CommunicationTypesLoader.get_comm_types()[comm_type]
                    ...
                    config.set(self.SETTINGS_COMM_MENU,
                               self.create_settings_key(controller.name, self.SETTINGS_KEY_COMM_STATUS),
                               '1' if comm_established else '0')
                    config.write()
    ...
    def on_start(self):
        ...
        if DEBUG_AUTO_CONNECT['connect']:
            if DEBUG_AUTO_CONNECT['type'] == 'serial':
                communication_port = CommunicationPortSerial(controller=self.controllers[0],
                                                             address=DEBUG_AUTO_CONNECT['address'])
